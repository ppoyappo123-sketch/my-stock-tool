...
if st.button("🔍 開始抓取"):
                ...
                for _, row in df_month.iterrows():
                    try:
                        ...
                        if '/' in date_str:
                            ...
                            if start_d <= ad_date <= end_d and str(row.iloc[1]).strip() == stock_id:
                                all_data.append({
    ...
    if all_data:
        ...
        st.success(f"✅ 成功抓取 **{len(df)} 筆不同日期** 資料！")
        st.dataframe(
        ...
        st.error("❌ 本次沒有抓到資料，請試其他股票或縮小日期範圍")

st.caption("月CSV穩定版 | 解決重複資料問題")
