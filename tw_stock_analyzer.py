...
mode = st.sidebar.selectbox(
    "請選擇分析模式", 
    ["大盤多日數據分析", "上市個股分析 (原版證交所)", "上櫃個股分析 (FinMind引擎)"],
    key="nav_v21" 
)
            ...
            for i, d in enumerate(date_list):
                ...
                if y and v and v.get('stat') == 'OK' and 'data' in v:
                    ...
                    all_results.append({
                        ...
                        '13:30累積金額(百萬)': row[7], formula_label: round(score, 4)
                    })
                progress.progress((i + 1) / len(date_list)); time.sleep(random.uniform(1.0, 1.5))
            status.empty()
            if all_results:
...
# --- 5. 模式 B：上市個股分析 ---
    ...
    if st.button("🚀 開始上市分析"):
        ...
        while temp_date <= end_date:
            res = fetch_json(f"https://www.twse.com.tw/exchangeReport/STOCK_DAY?response=json&date={temp_date.strftime('%Y%m%d')}&stockNo={stock_id}")
            ...
            temp_date += relativedelta(months=1); time.sleep(1.5)
...
# --- 6. 模式 C：上櫃個股分析 (修正 KeyError 版) ---
else:
    st.title("📉 上櫃個股分析 (FinMind 穩定引擎)")
    if not FINMIND_AVAILABLE:
        st.error("⚠️ 請確保環境已安裝 FinMind 套件。"); st.stop()
    ...
    if st.button("🔍 執行數據分析"):
        try:
            ...
            # 關鍵修正：檢查 df_raw 是否為 None 或是否包含必要的 "data" 鍵值對應結果
            if df_raw is not None and not df_raw.empty:
                ...
                st.info(f"📊 範圍平均指標：{avg_score:.4f} | 三倍門檻：{threshold:.4f}")
                
                # 套用精簡顯示欄位與紅字
                final_df = df[['日期', '最高', '最低', '收盤', '成交金額(億元)', formula_label]]
                st.dataframe(final_df.style.apply(lambda r: ['color:red;font-weight:bold' if r[formula_label] > threshold else '' for _ in r], axis=1), width='stretch')
            else:
                st.warning("⚠️ 該代號在此區間查無數據。")
        except Exception as e:
            st.error(f"❌ 發生未知錯誤：{str(e)}")
            st.info("💡 可能是 FinMind API 暫時性連線問題，請稍候再按一次「執行分析」。")
